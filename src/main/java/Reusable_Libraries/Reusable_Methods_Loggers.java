...
public class Reusable_Methods_Loggers
{
    ...
    {
        //kill all chrome driver instance (just in case) before opening a new one
        Thread.sleep(1000);
    ...
    //method to click on any button
    public static void clickByPartialLinkText(WebDriver driver, String locator, ExtentTest logger, String elementName)
    {
        //define explicit wait
        WebDriverWait wait = new WebDriverWait(driver,timeOut);
        ...
        try
        {
            System.out.println("\nClicking on " + elementName);
            logger.log(LogStatus.INFO,"Clicking on " + elementName);
            wait.until(ExpectedConditions.presenceOfElementLocated(By.partialLinkText(locator))).click();
        }
        catch (Exception err)
        {
            System.out.println("\nUnable to click on " + elementName + err);
